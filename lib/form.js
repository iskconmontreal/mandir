...
import { DON_CATS, EXP_CATS, DON_METHODS, INCOME_TYPES, AMT_MAX, DON_FIELDS, EXP_FIELDS, fmtStatus, fmtDate, eventBadge, eventLabel, eventDetail, actorName, setErr, buildMetaHistory, expenseTitle, expenseModalTitle, incomeModalTitle } from './finance.js'
...
export const emptyExpForm = () => ({ payee: '', amount: '', category: '', expense_date: '', note: '', member_id: null })
...
export function buildExpTimeline(s) {
  return [...(s.expHistory || [])].sort((a, b) => new Date(a.date) - new Date(b.date))
}
...
export function bindForm(s, { api, onSave }) {
  let pendingExtracts = 0, _syncedAmt = '', formSession = 0, receiptUid = 0
  const _today = today()
  ...
  function resetForm() {
    ...
    s.actionNote = ''; s.payRef = ''; s.expRejecting = false
  ...
  function syncExpReceipts() {
    ...
    const mismatches = []
    if (!dirtyFields.has('amount') && newAmt && (s.expForm.amount === _syncedAmt || !s.expForm.amount)) {
      s.expForm.amount = newAmt; _syncedAmt = newAmt
    } else if (newAmt && s.expForm.amount && parseFloat(s.expForm.amount) !== total) {
      mismatches.push(`amount: $${newAmt}`)
    }
    const first = s.receipts.find(r => r.data)?.data
    if (!first) return
    if (first.vendor || first.category || first.date) s.expFull = true
    if (!dirtyFields.has('payee') && !s.expForm.payee && first.vendor) s.expForm.payee = first.vendor
    else if (s.expForm.payee && first.vendor && s.expForm.payee !== first.vendor) mismatches.push(`vendor: ${first.vendor}`)
    if (!dirtyFields.has('category') && !s.expForm.category && first.category) s.expForm.category = first.category
    else if (s.expForm.category && first.category && s.expForm.category !== first.category) mismatches.push(`category: ${first.category}`)
    if (!dirtyFields.has('date') && !s.expForm.expense_date && first.date) s.expForm.expense_date = first.date
    else if (s.expForm.expense_date && first.date && s.expForm.expense_date !== first.date) mismatches.push(`date: ${first.date}`)
  ...
  s.payExp = async () => {
    await expAction(async () => { await api.payExpense(s.editId, s.payRef.trim()); return 'Expense marked as paid' })
  }
