...
export function annotateCarts(carts) {
  return carts.map(raw => {
    const due = raw.due_cents || 0
    const collected = raw.collected_cents || 0
    const templeDonation = raw.temple_donation_cents || 0
    const overpayment = raw.overpayment_cents != null
      ? raw.overpayment_cents
      : Math.max(0, collected - due)
    return {
      ...raw,
      donation: overpayment,
      itemsTotal: due - templeDonation,
      templeDonation,
      overpayment,
    }
  })
...
// Sums merchandise/food revenue (excludes Temple Donation and overpayment)
// for carts whose occurred_at falls in monthKey ("YYYY-MM").
export function monthShopSales(carts, monthKey) {
  let sum = 0
  for (const c of carts) {
    if (localMonthKey(c.occurred_at) !== monthKey) continue
    sum += (c.due_cents || 0) - (c.temple_donation_cents || 0)
  }
  return sum
}
