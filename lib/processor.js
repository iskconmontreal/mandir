...
export function setupProcessor(sprae, shared, { extras, onInit, onPropChange } = {}) {
  ...
  DEL.processor = (root, state) => {
    ...
    let sp = sprae(root, scope)

    for (let k in own) if (typeof own[k] === 'function') sp[k] = own[k].bind(sp)

    // Write back lifecycle hooks so define-element calls the bound version
    if (sp.onconnected) state.host.onconnected = sp.onconnected
    if (sp.ondisconnected) state.host.ondisconnected = sp.ondisconnected
