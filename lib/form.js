...
import { DON_CATS, EXP_CATS, DON_METHODS, AMT_MAX, DON_FIELDS, EXP_FIELDS, fmtStatus, fmtDate, eventBadge, eventLabel, eventDetail, actorName, setErr, buildMetaHistory } from './finance.js'
...
export function expEditPolicy(s) {
  ...
  const lockedAll = s.expMode === 'edit' && status === 'rejected'
  const amountLocked = s.expMode === 'edit' && (status === 'paid' || status === 'rejected')
  const payeeLocked = s.expMode === 'edit' && (status === 'paid' || status === 'rejected')
  const dateLocked = s.expMode === 'edit' && (status === 'paid' || status === 'rejected')
  const message = lockedAll
    ...
    : status === 'paid' && s.expMode === 'edit'
      ...
      : ''
  return { status, lockedAll, amountLocked, payeeLocked, dateLocked, message }
}
...
function metaEntries(item, names, myUid) {
...
export function bindForm(s, { api, onSave }) {
  ...
  function syncDonationControls() {
    requestAnimationFrame(() => {
      const root = activeModalRoot('don-donor', 'don-method', 'don-cat', 'don-date')
  ...
  function syncExpenseControls() {
    requestAnimationFrame(() => {
      ...
      const wantPayee = s.expForm.member_id ? (s.memberQuery || s.expForm.payee || '') : (s.expForm.payee || '')
  ...
  async function hydrateExpenseSession(id, session) {
    ...
    if (detail) {
      ...
      if (!s.expDirty) {
        if (!s.expForm.member_id && detail.member_id) s.expForm.member_id = detail.member_id || null
        if (!s.expForm.payee) s.expForm.payee = detail.payee || ''
        if (!s.expForm.amount) s.expForm.amount = rawAmt(detail.amount)
        if (!s.expForm.category) s.expForm.category = cat
        if (!s.expForm.expense_date) s.expForm.expense_date = (detail.expense_date || '').slice(0, 10)
        if (!s.expForm.note) s.expForm.note = detail.note || detail.description || ''
        if (!s.memberQuery && detail.member_id) s.memberQuery = s.memberNames[detail.member_id] || detail.payee || ''
      }
      s.expHistory = metaEntries(detail, s.userNames, s.myUid)
        // Removed syncExpenseControls() call to prevent value clobber
    }
    s.expApprovals = approvals.items || approvals || []
  }

  function openExpense(x, mode = 'edit', returnMode = 'close') {
    s.showAdd = false
    resetForm()
    ...
    requestAnimationFrame(() => {
      ...
      if (mode === 'detail') hydrateExpenseSession(x.id, session).catch(() => {})
    })
  }
  ...
  s.viewExpense = x => openExpense(x, 'detail', 'close')
  ...
  s.viewDonation = d => {
    ...
    s.donFull = true
    s.showAdd = true
    s.donHistory = metaEntries(d, s.userNames, s.myUid)
  ...
  s.cancelDonationEdit = () => {
    if (s.detailDon) return s.viewDonation(s.detailDon)
    s.cancelAdd()
  }

  s.cancelExpenseEdit = () => {
    if (s.expReturnMode === 'detail' && s.detailExp) return s.viewExpense(s.detailExp)
    s.cancelAdd()
  }
