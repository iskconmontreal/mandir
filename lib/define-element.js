...
function scopeCSS(css, tag) {
  // extract :host rules' bodies to avoid double-nesting (tag { tag { ... } })
  let hostRules = []
  css = css.replace(/:host\b(?:\(([^)]*)\))?\s*\{([^}]*)\}/g, (_, sel, body) => {
    hostRules.push(sel ? `${tag}${sel} { ${body} }` : `${tag} { ${body} }`)
    return ''
  })
  css = css.trim()
  return hostRules.join('\n') + (css ? `\n${tag} { ${css} }` : '')
}
...
Object.defineProperty(DefineElement, 'processor', {
  ...
  set: (fn) => {
    ...
    for (let el of _noProc) if (el.isConnected) { el._render(); el.onconnected?.() }
    _noProc.clear()
  }
})
