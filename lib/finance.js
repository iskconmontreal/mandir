...
export const EXP_FIELDS = { Payee: { id: 'vendor', label: 'payee' }, Amount: { id: 'amount', label: 'amount' }, Category: { id: 'cat', label: 'category' }, ExpenseDate: { id: 'date', label: 'date' }, Note: { id: 'desc', label: 'note' } }
...
export function eventDetail(h) {
  ...
  for (const [k, v] of Object.entries(d)) {
    ...
    else if (k === 'reference' && v) parts.push('Ref ' + v)
  }
  return parts.join(' · ')
}
...
export function buildMetaHistory(item) {
  ...
  if (item.updated_at && item.updated_at !== item.created_at) rows.push({ action: 'update', user_id: item.updated_by || null, created_at: item.updated_at })
