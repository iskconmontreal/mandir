...
export const BASE = localStorage.getItem('mandir_api') || 'https://api.iskconmontrealsankirtan.com'
...
// Abort requests that never complete (dead host, hung server) so buttons show a
// clear error instead of spinning forever. Generous cap — imports can be slow.
...
async function request(path, opts = {}, retry = false) {
  ...
  try {
    ...
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      throw new Error(`Request timed out — the server at ${BASE} did not respond`)
    }
  ...
  const emptyOk = res.status === 204 || (res.status === 202 && !ct.includes('application/json'))
  if (!ct.includes('application/json') && !emptyOk) {
...
export const api = {
  ...
  getBackupStatus: () => api.get('/api/admin/backup/status'),
  runBackup:       () => api.post('/api/admin/backup/run'),
  restoreBackup:   () => api.post('/api/admin/backup/restore'),
  ...
  getSankirtanBooks:        ()                  => api.get('/api/sankirtan/books'),
  createSankirtanBook:      data                => api.post('/api/sankirtan/books', data),
  updateSankirtanBook:      (id, d)             => api.put(`/api/sankirtan/books/${id}`, d),
  deleteSankirtanBook:      id                  => api.del(`/api/sankirtan/books/${id}`),
  // Bulk catalog import: multipart upload (FormData skips the JSON Content-Type).
  importSankirtanBooks: file => {
    const form = new FormData()
    form.append('file', file)
    return request('/api/sankirtan/books/import', { method: 'POST', body: form })
  },
  // Per-book cover image (server stores/serves WebP; t = cache-bust after upload).
  sankirtanBookCoverUrl: (id, t) => `${BASE}/uploads/sankirtan/covers/${id}.webp${t ? `?t=${t}` : ''}`,
  uploadSankirtanBookCover: (id, file) => {
    const form = new FormData()
    form.append('cover', file)
    return request(`/api/sankirtan/books/${id}/cover`, { method: 'POST', body: form })
  },
  deleteSankirtanBookCover: id => api.del(`/api/sankirtan/books/${id}/cover`),
  // Extra pictures (gallery): server returns upload paths; prefix with the goloka origin.
  sankirtanBookImageUrl: (path, t) => `${BASE}${path}${t ? `?t=${t}` : ''}`,
  getSankirtanBookImages: id => api.get(`/api/sankirtan/books/${id}/images`),
  uploadSankirtanBookImage: (id, file) => {
    const form = new FormData()
    form.append('image', file)
    return request(`/api/sankirtan/books/${id}/images`, { method: 'POST', body: form })
  },
  deleteSankirtanBookImage: (id, slot) => api.del(`/api/sankirtan/books/${id}/images/${slot}`),
  // Public storefront catalog (unauthenticated) — only website-ready books
  // (active + priced + description + cover); used for the "On website" badges.
  getPublicSankirtanBooks: () => api.get('/api/public/books'),
  // Binary download (see downloadSankirtanReportXLSX): fetch the Blob ourselves.
  downloadSankirtanCatalogXLSX: async () => {
    const headers = {}
    if (auth.token) headers['Authorization'] = `Bearer ${auth.token}`
    const res = await fetch(`${BASE}/api/sankirtan/books/export.xlsx`, { headers })
    ...
    a.download = match ? match[1] : 'sankirtan-catalog.xlsx'
    a.click()
    URL.revokeObjectURL(a.href)
  },

  getSankirtanInventory:    ()                  => api.get('/api/sankirtan/inventory'),

  getSankirtanOrders:       ()                  => api.get('/api/sankirtan/orders'),
  createSankirtanOrder:     items               => api.post('/api/sankirtan/orders', { items }),
  updateSankirtanOrder:     (id, items)         => api.put(`/api/sankirtan/orders/${id}`, { items }),
  setSankirtanOrderStatus:  (id, status)        => api.post(`/api/sankirtan/orders/${id}/status`, { status }),
  receiveSankirtanOrder:    (id, items)         => api.post(`/api/sankirtan/orders/${id}/receive`, { items }),
  downloadSankirtanOrderXLSX: async (id) => {
    const headers = {}
    if (auth.token) headers['Authorization'] = `Bearer ${auth.token}`
    const res = await fetch(`${BASE}/api/sankirtan/orders/${id}/export.xlsx`, { headers })
    if (!res.ok) throw new Error(res.statusText)
    const blob = await res.blob()
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `book-order-${id}.xlsx`
    a.click()
    URL.revokeObjectURL(a.href)
  },

  getSankirtanSessions:     p                   => api.get(`/api/sankirtan/sessions?${new URLSearchParams(p || {})}`),
  getSankirtanSession:      id                  => api.get(`/api/sankirtan/sessions/${id}`),
  updateSankirtanSession:   (id, d)             => api.put(`/api/sankirtan/sessions/${id}`, d),
  deleteSankirtanSession:   id                  => api.del(`/api/sankirtan/sessions/${id}`),
  // Sessions backup/restore (manager-only): multipart JSON import + binary download,
  // mirroring the catalog import/export above.
  importSankirtanSessions: (file, adjustStock, consolidate) => {
    const form = new FormData()
    form.append('file', file)
    form.append('adjust_stock', adjustStock ? 'true' : 'false')
    form.append('consolidate', consolidate ? 'true' : 'false')
    return request('/api/sankirtan/sessions-import', { method: 'POST', body: form })
  },
  // ids: optional array of session ids — exports only those; empty/absent → all.
  downloadSankirtanSessionsBackup: async (ids) => {
    const headers = {}
    if (auth.token) headers['Authorization'] = `Bearer ${auth.token}`
    const qs = ids && ids.length ? `?ids=${ids.join(',')}` : ''
    const res = await fetch(`${BASE}/api/sankirtan/sessions-export.json${qs}`, { headers })
    if (!res.ok) throw new Error(res.statusText)
    const blob = await res.blob()
    const cd = res.headers.get('content-disposition') || ''
    const match = cd.match(/filename="?([^"]+)"?/)
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = match ? match[1] : 'sankirtan-sessions-backup.json'
    a.click()
    URL.revokeObjectURL(a.href)
  },
  getSankirtanLeaderboard:  period              => api.get(`/api/sankirtan/leaderboard?period=${period}`),

  // Events register (public website: next event + past photo albums).
  listSankirtanEvents:      ()                  => api.get('/api/sankirtan/events'),
  createSankirtanEvent:     data                => api.post('/api/sankirtan/events', data),
  updateSankirtanEvent:     (id, d)             => api.put(`/api/sankirtan/events/${id}`, d),
  deleteSankirtanEvent:     id                  => api.del(`/api/sankirtan/events/${id}`),
  listSankirtanEventPhotos: id                  => api.get(`/api/sankirtan/events/${id}/photos`),
  uploadSankirtanEventPhoto: (id, file) => {
    const form = new FormData()
    form.append('photo', file)
    return request(`/api/sankirtan/events/${id}/photos`, { method: 'POST', body: form })
  },
  reorderSankirtanEventPhotos: (id, ids)        => api.put(`/api/sankirtan/events/${id}/photos/order`, { ids }),
  deleteSankirtanEventPhoto: (id, photoId)      => api.del(`/api/sankirtan/events/${id}/photos/${photoId}`),
  // Photo paths come from goloka (like cover_url); prefix with the goloka origin.
  sankirtanEventPhotoUrl:   (path, t)           => `${BASE}${path}${t ? `?t=${t}` : ''}`,

  getSankirtanReport:       (from, to)          => api.get(`/api/sankirtan/report?${new URLSearchParams({ from: from || '', to: to || '' })}`),
  sendSankirtanReportEmail: payload             => api.post('/api/sankirtan/report/email', payload),
  // Binary download: request() is JSON-only, so fetch the Blob ourselves (Bearer
  // added manually, like fetchHtml) and trigger a download via a temporary <a>.
  downloadSankirtanReportXLSX: async (from, to) => {
    const headers = {}
    if (auth.token) headers['Authorization'] = `Bearer ${auth.token}`
    const qs = new URLSearchParams({ from: from || '', to: to || '' })
    const res = await fetch(`${BASE}/api/sankirtan/report.xlsx?${qs}`, { headers })
    if (!res.ok) throw new Error(res.statusText)
    const blob = await res.blob()
    const cd = res.headers.get('content-disposition') || ''
    const match = cd.match(/filename="?([^"]+)"?/)
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = match ? match[1] : 'BBT-Score.xlsx'
    a.click()
    URL.revokeObjectURL(a.href)
  },
